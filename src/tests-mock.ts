...
async function runNode(
  ...
  if (node.type === "suite") {
    testStatesEmitter.fire({ type: "suite", suite: node.id, state: "running" } as TestSuiteEvent);

    for (const child of node.children) {
      await runNode(child, testStatesEmitter);
    }

    testStatesEmitter.fire({ type: "suite", suite: node.id, state: "completed" } as TestSuiteEvent);
  } else {
    // node.type === 'test'

    testStatesEmitter.fire({ type: "test", test: node.id, state: "running" } as TestEvent);

    testStatesEmitter.fire({ type: "test", test: node.id, state: "passed" } as TestEvent);
  }
}
